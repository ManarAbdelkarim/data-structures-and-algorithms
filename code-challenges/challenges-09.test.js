...
const addValues = (arr) => {
  const nweArray =arr.reduce((accumalator, val) => accumalator += val, 0);
  return nweArray;
};
...
const addPurchases = (arr) => {
  const newArray= arr.reduce((accumalator, item) => accumalator += item.purchasePrice, 0);
  return newArray;
};
...
const returnNames = (arr) => {
  const newArray= Object.values(arr.reduce((acc, obj, idx) => {
    acc[idx] = obj.name;
    return acc;
  }, {}));
  return newArray;
