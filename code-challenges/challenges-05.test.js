...
const sortStarWarsCharacters = (starWarsArr) => {
  ...
  starWarsArr.sort((a, b) => parseFloat(b.height) - parseFloat(a.height));

  console.log(starWarsArr[0]['name']);
  console.log(starWarsArr[2]['height']);
  return starWarsArr;
};
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(element => {
    const sOne = element.split(' ');
    const sTwo = sOne.slice(2);
    const sThree = sTwo.join(' ');
    result.push(sThree);
  });
  return result;
};
